        ...
        return [
            'access' => [
                ...
                'rules' => [
                    [
                        'actions' => [
                            'signup', 'login', 'request-password-reset', 'reset-password', 'oauth', 'activation', 'public'
                        ],
                        'allow' => true,
                        'roles' => ['?']
                    ],
                    [
                        ...
                        'denyCallback' => function () {
                            return Yii::$app->controller->redirect(['/user/profile/']);
                        }
                    ],
                    [
                        'actions' => ['logout'],
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            if ($user) {
                if ($model->shouldBeActivated()) {
                    ...
                    Yii::$app->getSession()->setFlash('notice', Yii::t('frontend', 'Your account has been successfully created. Check your email for further instructions.'));
    ...
    public function actionRequestPasswordReset()
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->validate()) {
            if ($model->sendEmail()) {
                ...
                Yii::$app->getSession()->setFlash('warning', Yii::t('frontend', 'Sorry, we are unable to reset password for email provided.'));
    ...
    public function successOAuthCallback($client)
    {
        ...
        if (!$user) {
            ...
            if ($user->save()) {
                ...
                if ($sentSuccess) {
                    Yii::$app->getSession()->setFlash('success', Yii::t('frontend', 'Welcome to {app-name}. Email with your login information was sent to your email.', [
                ...
                if ($user->email && User::find()->where(['email' => $user->email])->count()) {
                    Yii::$app->getSession()->setFlash('notice', Yii::t('frontend', 'We already have a user with email {email}', [
                        'email' => $user->email
                    ]));

                } else {
                    Yii::$app->getSession()->setFlash('warning', Yii::t('frontend', 'Error while oauth process.'));
